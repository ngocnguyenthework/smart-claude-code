#!/usr/bin/env node
/**
 * Shared hook enable/disable controls for smart-claude.
 *
 * Controls:
 * - SC_HOOK_PROFILE=minimal|standard|strict (default: standard)
 * - SC_DISABLED_HOOKS=comma,separated,hook,ids
 */
...
function getHookProfile() {
  const raw = String(process.env.SC_HOOK_PROFILE || 'standard').trim().toLowerCase();
  return VALID_PROFILES.has(raw) ? raw : 'standard';
}

function getDisabledHookIds() {
  const raw = String(process.env.SC_DISABLED_HOOKS || '');
