#!/usr/bin/env node
/**
 * Executes a hook script only when enabled by smart-claude hook profile flags.
 *
 * Usage:
 *   node run-with-flags.js <hookId> <scriptRelativePath> [profilesCsv]
 *
 * Resolves scripts relative to the plugin root (CLAUDE_PLUGIN_ROOT env var
 * or two levels above this file). Supports both fast `module.exports.run()`
 * hooks and legacy stdin-reading scripts via spawnSync fallback.
 */
...
async function main() {
  ...
  const result = spawnSync(process.execPath, [scriptPath], {
    ...
    env: {
      ...process.env,
      SC_HOOK_INPUT_TRUNCATED: truncated ? '1' : '0',
      SC_HOOK_INPUT_MAX_BYTES: String(MAX_STDIN)
    },
    cwd: process.cwd(),
    timeout: 30000
  });
